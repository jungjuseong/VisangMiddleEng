(function(app_o, comm_o, tsoc_o){
	...
	function _load(){
		return new Promise(function(resolve, reject){
			app_o.importLink('/content/digenglishCB_lib/D/word_english/teacher/import.html').then(
				function(doc){
					...
					/*
					var cnt = 2;
					app_o.importHTML('/content/digenglishCB_lib/BASIC/voca/common/ball.svg').then(
						function(el){
							cnt--;
					...
					app_o.importHTML('/content/digenglishCB_lib/BASIC/voca/common/egg.svg').then(
						function(el){
							cnt--;
							var page_quiz = document.getElementById("page_quiz");
					...
					if(_isDvlp){
						...
						if(tsoc_o){
							...
							tsoc_o.getStudents(function(arr, isOk){
								...
								if(isOk){
									app_o.init(arr, "Vocaburary", "Lesson1", true, true);
									app_o.start();
							...
							*/
							...
							app_o.init(students, "Vocabulary", "Lesson1", true, true);
							app_o.start();
						}
						...
						tsoc_o.getStudents(function(arr, isOk){
							...
							tsoc_o.initComplete(
								function(step, lesson){
									if(!lesson || lesson=="") lesson = "Vocaburary";
									if(!step || step=="") step = "Lesson1";
